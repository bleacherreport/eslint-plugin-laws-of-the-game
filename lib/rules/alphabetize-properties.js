  ...
  create: function(context) {
    ...
    function checkObject(node) {
      ...
      if (propertyKeys.find(firstKeyOutOfOrder)) {
        context.report({message: "Objects with "+limit+" or more properties should have properties in alphabetical order. (Saw "+propertyKeys.length+": "+propertyKeys.join(",")+")", node: node});
      }
    }
    return {
